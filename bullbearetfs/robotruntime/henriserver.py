...
from bullbearetfs.models import AcquisitionPolicy, StartupStatus, EquityTradingRobot, TradeDataHolder
from bullbearetfs.utilities.jms import EggheadJMSListenerWrapper
from bullbearetfs.utilities.alpaca import AlpacaAPIBase
...
# Write into a Startup Table. Know about the uptime of the system.
  ...
  @sync_to_async
  def getActiveTradingRobots(self):
    ...
    for robot in all_robots:
      statement.append(robot.prepareTrades())
...
async def main(manager):
  while manager.continueTrading():
    ...
    statement = await manager.getActiveTradingRobots()
