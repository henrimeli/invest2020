...
#Options for Chrome: https://stackoverflow.com/questions/55012755/how-to-set-window-size-in-selenium-chrome-python

class TestFoundationBrowser(StaticLiveServerTestCase):
  testname ='TestDefaultBrowser'
...
class TestChromeBrowser(StaticLiveServerTestCase):
  testname ='TestChromeBrowser'

  @classmethod 
  def setUp(self):
    self.browser = EggheadSeleniumBrowser(browser_type='chrome')
    self.driver = self.browser.getDriver()
...
@unittest.skip("Taking a break now")
class TestSafariBrowser(StaticLiveServerTestCase):
  testname ='TestSafariBrowser'

  @classmethod 
  def setUp(self):
    self.browser = EggheadSeleniumBrowser(browser_type='safari')
    self.driver = self.browser.getDriver()
