...
##################################################################################
  ...
  def recordDispositionTransaction(robot,order,transition_root_id=None):
    ...
    ETFPairRobotExecutionData.recordDispositionExecution(executor=robot.execution_engine,exec_time=sell_order['sell_date'],
                                   order_client_id=sell_order['sell_order_client_id'],exec_params=robot.getRobotExecutionParams(),income=0)
...
##################################################################################
  ...
  def recordAcquisitionTransaction(robot,bullish,bearish):
    ...
    bull_entry=TradeDataHolder.objects.create(robot=robot, symbol=bullish['symbol'], buy_price=bullish['price'],
                             ...
                             quantity=bullish['qty'],execution=robot.execution_engine )
    ...
    bear_entry=TradeDataHolder.objects.create(robot=robot, symbol=bearish['symbol'], buy_price=bearish['price'],
                             ...
                             quantity=bearish['qty'],execution=robot.execution_engine)

    #Record the Bull Purchase transaction with the Execution Engine:
    ETFPairRobotExecutionData.recordAcquisitionExecution(executor=robot.execution_engine,exec_time=bullish['date'],
                                   order_client_id=bullish['bull_buy_order_client_id'],exec_params=robot.getRobotExecutionParams(),cost=0)

    #Record the Bear Purchase transaction with the Execution Engine:
    ETFPairRobotExecutionData.recordAcquisitionExecution(executor=robot.execution_engine,exec_time=bearish['date'],
                                   order_client_id=bearish['bear_buy_order_client_id'],exec_params=robot.getRobotExecutionParams(),cost=0)

    robot_id = robot.id
    exec_engine_id = None if robot.execution_engine==None else robot.execution_engine.id
